...
class Uploader
{
	...
	function extendFile($filepath){
		if(strpos($filepath,'.')!==false){
			require_once('../../../../../FrPHP/Extend/'.$filepath);
		}else{
			$Extend = scandir('../../../../../FrPHP/Extend'.$filepath);
			
			foreach($Extend as $v){
				if(strpos($v,'.php')!==false){
					require_once '../../../../../FrPHP/Extend/'.$filepath.'/'.$v;
				}
			}
		}
	...
	public function save_files($filename){
		...
		if($config['db']['type']=='mysql'){
			...
			$plugin = $db->set_table('plugins');
			$plugins = $plugin->find(['filepath'=>'jzuploads','isopen'=>1]);
			if($plugins){
				
				$config = json_decode($plugins['config'],1);
				
				$this->extendFile('/uploads');
				$pic = $file_path;
				...
				$this->fullName = $litpic;
				$pictures = $db->set_table('pictures');
				$pictures->add(['litpic'=>$litpic,'addtime'=>time(),'userid'=>0,'size'=>$filesize,'filetype'=>str_replace('.','',$this->fileType)]);
			...
			$sqlite = '../../../../../'.$config['db']['dbpath'].'/'.$config['db']['dbname'].'.db';
			...
			//进程存储处理
			//将传入表数据的对象类放入一个变量中，方便多次使用
			$table = $config['db']['prefix'].'plugins';
			$sql = " selet * from ".$table." where filepath='jzuploads' and isopen=1 ";
			$ret = $db->query($sql);
			while($rows[] = $ret->fetchArray(SQLITE3_ASSOC)){}
			array_pop($rows);
			$plugins = $rows[0];
			//$plugins = $db->find(['filepath'=>'jzuploads','isopen'=>1]);
			if($plugins){
				...
				$row = ['litpic'=>$litpic,'addtime'=>time(),'userid'=>0,'size'=>$filesize,'filetype'=>str_replace('.','',$this->fileType)];
				...
				$col = join(',', $cols);
				$val = join(',', $vals);
				$table = $config['db']['prefix'].'pictures';
