...
class QqController extends CommonController
{
	  ...
	  function index(){
		   ...
		   if($this->frparam('state',1) == $_SESSION['state'])
		   {
		    ...
		    //Step4：使用<span >openid,</span><span >access_token来获取所接受的用户信息。</span>
		    	...
		    	$user_data = curl_http($user_data_url);//此为获取到的user信息
		    	//var_dump($user_data);
		    	...
		    	$res = M('member')->find(array('qq_openid'=>$user->openid));
			  	if($res){
					...
					//检查邮箱是否绑定
					if(!$res['email']){
						setCache('qq_openid',$user->openid,120);
						$this->display('@'.APP_PATH.'static/common/bindemail.html');
						exit;
					}
	...
	function bindemail(){
		if(getCache('qq_openid')){
			$openid = getCache('qq_openid');
			...
			//检查邮箱是否已存在
			if(M('member')->find(['email'=>$email])){
				Error('邮箱已存在！');
			}
			
			$res = M('member')->find(['qq_openid'=>$openid]);
			if($res){
				M('member')->update(['qq_openid'=>$openid],['email'=>$email]);
				$group = M('member_group')->find(array('id'=>$res['gid']));
				if(!$group){
					Error('未找到您所在分组，请联系管理员处理！',U('Login/index'));
				}
			...
			Error('验证已过期，请重新登录！',U('login/index'));
		}
	}
